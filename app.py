...
def predict (b=all_columns):
    ...
    for i in to_predict_list:
        sentence = decontracted1(i)
        new.append(sentence)
    
    df_new=pd.DataFrame(new,columns=b,index =[1])
    
    df_new['PKT_TYPE_encode']=PKT_TYPE_LE.fit_transform(df_new['PKT_TYPE'])
    df_new['NODE_NAME_FROM_encode']=NODE_NAME_FROM_LE.fit_transform(df_new['NODE_NAME_FROM'])
    df_new['NODE_NAME_TO_encode']=NODE_NAME_TO_LE.fit_transform(df_new['NODE_NAME_TO'])
    df_new = df_new.drop(['PKT_TYPE','NODE_NAME_FROM','NODE_NAME_TO','FLAGS'], axis=1)
    
    for i in range(df_new.shape[1]):
        t=df_new.columns[i]
        new_val=scale[i].transform(np.float64(df_new.values[0][i]).reshape(-1, 1))
        df_new[t]=new_val
    result=loaded_model_1.predict(df_new)
    if result == 0:
        prediction= 'Normal traffic'
    else:
        df_new1=pd.DataFrame(new,columns=b,index =[1])
        ...
        for i in range(df_new1.shape[1]):
            t=df_new1.columns[i]
            new_val=scale_1[i].transform(np.float64(df_new1.values[0][i]).reshape(-1, 1))
            df_new1[t]=new_val
        result1=loaded_model_2.predict(df_new1)
        if result1== 0:
            prediction= 'UDP-Flood Attack'
        else:
            df_new2=pd.DataFrame(new,columns=b,index =[1])
            ...
            for i in range(df_new2.shape[1]):
                t=df_new2.columns[i]
                new_val=scale_2[i].transform(np.float64(df_new2.values[0][i]).reshape(-1, 1))
                df_new2[t]=new_val
